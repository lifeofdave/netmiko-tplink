...
class BaseConnection(object):
    ...
    def _read_channel_timing(self, delay_factor=1, max_loops=15):
        ...
        if delay_factor == 1 and max_loops == 15:
            max_loops = int(self.timeout / loop_delay)
    ...
    def set_terminal_width(self, command="", delay_factor=1):
        ...
        delay_factor = self.select_delay_factor(delay_factor)
        command = self.normalize_cmd(command)
        self.write_channel(command) # tries to write 'terminal width 511\n'
        output = self.read_until_prompt() # TODO this is
        if self.ansi_escape_codes:
            output = self.strip_ansi_escape_codes(output)
        return output
    ...
    def normalize_linefeeds(self, a_string):
        ...
        :type a_string: str
        Customised for tp-link switch
        """
        newline = re.compile("(\r\r\r\n|\r\r\n|\r\n)") # 'T1500G-10PS>\r\n\r\n\rT1500G-10PS>' Customised for TP-link
